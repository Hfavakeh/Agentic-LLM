...
# Prompt-representation arms (Email-8). Each changes ONE property of the
# rendered history; defaults reproduce the protocol payload exactly.
python main.py --payload-repr numeric        # measured numbers, no uncertainty
python main.py --payload-repr numeric_ci     # numbers + 3-training spread + per-seed
python main.py --payload-repr ranks          # ordering only, no magnitudes
python main.py --no-anchor                   # remove best-so-far (forces complete settings)
python main.py --history-window 5            # recent outcomes only vs all 25
python main.py --no-auto-conclusions         # observations without mined patterns/labels
...
- The LLM prompt is rendered as **qualitative labels** (best/good/poor, low/med/high, small/med/large, behavior labels), never raw numbers — small local models parse it more reliably and the protocol mandates it. Raw numbers stay in the per-setting CSV + JSON logs only. **The prompt and the input representation are experimental variables, not implementation details** (Email-8): the label rendering is itself under test, since it discards magnitude, ordering and the across-3-trainings uncertainty. Measured, not assumed — `scripts/dump_llm_prompts.py` audits any run and reports 0% of scores / stds / per-seed values reaching the payload as numbers. Every proposer call now records the **verbatim** system prompt and user message (including the retry variant) plus a full-fidelity numeric `history_snapshot`, so "what the LLM saw" is auditable from the logs rather than inferred from the code. The alternatives are selectable per run — `--payload-repr {labels,numeric,numeric_ci,ranks}`, `--no-anchor`, `--history-window N`, `--no-auto-conclusions` (`PAYLOAD_REPRS` in `arms/prompts.py`) — and `protocol_system_prompt` must stay in lock-step with `format_protocol_payload` via `_history_description`, or the prompt would describe information the payload does not carry. The `labels` default path is byte-identical to the pre-variant rendering (asserted against `git show HEAD` when it was added), so earlier runs stay comparable; `--no-anchor` additionally forces `validate_protocol_changes(require_complete=True)`, since the validator's merge would otherwise refill the unnamed HPs from the hidden anchor and leave the arm anchored after all.
